...
public final class QConnectorSyncImplReconnectTest extends ATest {
	
	@Test
	public void testConnectionFail() throws Exception {
		this.launchQProcess();
		final QConnectorSyncImpl c = new QConnectorSyncImpl("localhost", 5000, false);
		c.connect();
	...
	public void testConnectionFailWithReconnect() throws Exception {
		this.launchQProcess();
		final QConnectorSyncImpl c = new QConnectorSyncImpl("localhost", 5000, true);
		c.connect();
		Result r1 = c.execute("1+1");
		Assert.assertTrue(r1 instanceof PrimitiveResult);
		this.terminateQProcess();
